# Kramaniti Clarity Square Manager App

## Purpose

[Recommendation] This is the second ChatGPT Apps SDK app for Kramaniti. It is a private Clarity Square manager for signed-in workspace data.
...
[Fact] Clarity Square data already lives in Supabase under the `clarity_square` schema with RLS ownership policies.
...
[Recommendation] ChatGPT should use OAuth. The OAuth login page accepts the user's Clarity Square username or email plus password, verifies it through Supabase Auth, then returns a short-lived encrypted app token to ChatGPT.
...
```bash
cd website/clarity-square-chatgpt-app
npm install
npm run typecheck
npm start
```
...
[Recommendation] Target Vercel project after the rename:

```text
clarity-square-chatgpt-app
```

[Recommendation] Target production MCP URL after the Vercel project or alias is updated:

```text
https://clarity-square-chatgpt-app.vercel.app/mcp
```
...
Target production URL after deployment or alias update:
...
[Constraint] A real signed-in data read/write test still requires signing in through the ChatGPT OAuth flow with an actual Clarity Square account.
