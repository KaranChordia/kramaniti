...
- Settings includes a Delete Data section listing saved context, projects, tasks, memories, folders, and assistant threads. Deletes are item-level, confirmation-gated, update local UI immediately, and persist through the existing signed-in Supabase tables when available.
