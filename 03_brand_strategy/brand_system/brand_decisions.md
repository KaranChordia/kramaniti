...
| 2026-08-21 | Implemented `[Fact]` | Use a square-first supporting language for new website and brand-system work, with the current hero and floating navigation as the reference implementation. Retire circular/orbital supporting motifs from new work while preserving the official mark. | The founder selected the faster, restrained hero direction and explicitly replaced circular objects with square connectors. Orthogonal rails, travelling signals, square handoffs, and 8 px control geometry make the evolved system repeatable without imposing an immediate site-wide rewrite. | `08_brand_assets/exports/kramaniti_website_design_kit.pdf`, `scripts/render_kramaniti_website_design_kit.py`, `09_reviews/decisions.md` |
