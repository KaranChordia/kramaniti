-- Kramaniti Clarity Square auth profile trigger
-- Ensures every Clarity Square auth signup creates a matching isolated profile row.
...
drop trigger if exists clarity_square_create_profile_on_auth_signup on auth.users;
create trigger clarity_square_create_profile_on_auth_signup
after insert on auth.users
for each row execute function clarity_square.handle_new_auth_user();
...
select
  ...
  users.email,
  case
    when clarity_square.normalized_username(users.raw_user_meta_data ->> 'username') ~ '^[a-z0-9_]{3,24}$'
      then clarity_square.normalized_username(users.raw_user_meta_data ->> 'username')
    else null
  end
from auth.users
