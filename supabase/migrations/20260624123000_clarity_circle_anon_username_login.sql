...
language plpgsql
security definer
set search_path = pg_catalog, clarity_circle, public
as $$
declare
  ...
  matched_email text;
begin
  normalized_login := clarity_circle.normalized_username(login);
  ...
  if matched_user_id is not null then
    ...
    if matched_email is not null then
      update clarity_circle.profiles
  ...
  into matched_user_id, matched_email
  from auth.users
  where clarity_circle.normalized_username(users.raw_user_meta_data ->> 'username') = normalized_login
    and users.email is not null
