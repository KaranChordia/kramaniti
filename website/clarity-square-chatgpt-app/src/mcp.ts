...
const WORKSPACE_URI = "ui://kramaniti/clarity-square-manager.html";
...
const readSecuritySchemes = [{ type: "oauth2", scopes: ["clarity_square.read"] }] as const;
const writeSecuritySchemes = [{ type: "oauth2", scopes: ["clarity_square.read", "clarity_square.write"] }] as const;
...
async function authClient(headers: Headers) {
  ...
  return createSquareClient(supabaseAccessToken);
}

function authResponse(message: string): ManagerOutput {
  return {
    mode: "auth_required",
    title: "Connect your Clarity Square account",
    summary: message,
    stats: [],
    projects: [],
    tasks: [],
    ...
    draft: null,
    guardrails: [
      "This manager only works after the user connects a Clarity Square account through OAuth.",
...
function authChallengeResponse(headers: Headers, message: string, scope = "clarity_square.read"): ToolResponse {
  return {
    ...toolResponse(authResponse(message), "Connect your Clarity Square account to use this manager."),
    isError: true,
    _meta: {
...
async function loadWorkspace(client: SquareClient) {
...
async function withAuth(
  headers: Headers,
  fn: (client: SquareClient, userId: string) => Promise<ManagerOutput>,
  scope = "clarity_square.read"
...
function buildProjectDraft(input: {
  ...
  return {
    ...
    context: clean(input.roughIdea, "A new Clarity Square project."),
    project_instruction: [
      `Project: ${title}`,
      `Context: ${clean(input.roughIdea, "A new Clarity Square project.")}`,
...
export function createKramanitiMcpServer(requestHeaders = new Headers()): McpServer {
  const server = new McpServer({
    name: "kramaniti-clarity-square-manager",
  ...
  registerAppTool(
    ...
    "get_workspace_overview",
    {
      ...
      description: "Read the signed-in user's Clarity Square workspace and summarize projects, tasks, memories, reports, and items needing attention.",
      inputSchema: {},
      outputSchema,
    ...
    async () => withAuth(requestHeaders, async (client) => {
      ...
      return workspaceOutput("overview", "Clarity Square workspace", "Current signed-in workspace state from Supabase.", data);
    })
  );
  ...
  registerAppTool(
    ...
    {
      title: "List Projects",
      description: "List the signed-in user's active Clarity Square projects, optionally filtered by search text.",
    ...
    async ({ search }) => withAuth(requestHeaders, async (client) => {
      ...
      return workspaceOutput("projects", "Projects", term ? `Projects matching "${term}".` : "Active Clarity Square projects.", data);
    })
  );
  ...
  registerAppTool(
    ...
    "get_project_detail",
    {
      ...
      description: "Open one Clarity Square project with related tasks, memories, and reports.",
  ...
  registerAppTool(
    ...
    "create_project_draft",
    {
      ...
      description: "Draft a new Clarity Square project from a rough idea. This does not save until the user approves and a later write action is run.",
      inputSchema: {
    ...
    async (input) => withAuth(requestHeaders, async (client) => {
      const data = await loadWorkspace(client);
      return workspaceOutput("project_draft", "Project draft ready", "Review this draft before saving it to Clarity Square.", data, buildProjectDraft(input));
    })
  );
  ...
  registerAppTool(
    ...
    "list_project_tasks",
    {
      ...
      description: "List tasks for all projects or one selected Clarity Square project.",
      inputSchema: {
  ...
  registerAppTool(
    ...
    {
      title: "List Memories",
      description: "List active assistant memories for the signed-in user's Clarity Square workspace.",
      inputSchema: {
        projectId: z.string().optional(),
  ...
  registerAppTool(
    ...
    "save_memory_draft",
    {
      ...
      description: "Draft an assistant memory note. This does not save directly; review before adding it to Clarity Square.",
      inputSchema: {
        title: z.string(),
        content: z.string(),
    ...
    async (input) => withAuth(requestHeaders, async (client) => {
      const data = await loadWorkspace(client);
      return workspaceOutput("memory_draft", "Memory draft ready", "Review this memory before saving it to Clarity Square.", data, {
        title: input.title,
  ...
  registerAppResource(
    server,
    "Kramaniti Clarity Square Manager Widget",
    WORKSPACE_URI,
    {
      mimeType: RESOURCE_MIME_TYPE,
      description: "Interactive Clarity Square manager workspace HTML.",
    },
    async () => ({
      contents: [
        {
