...
const SCOPES = ["clarity_circle.read", "clarity_circle.write"];
...
export function wwwAuthenticateHeader(req: IncomingMessage, scope = "clarity_circle.read") {
...
export function wwwAuthenticateHeaderFromHeaders(headers: Headers, scope = "clarity_circle.read") {
...
<html lang="en">
  <head>
    ...
    <title>Connect Clarity Circle</title>
    <style>
  ...
  <body>
    <main>
      <h1>Connect Clarity Circle</h1>
      <p>Sign in with your Clarity Circle username or email. ChatGPT will only receive access scoped to this manager app.</p>
      ...
      <div class="note">No service-role key is used. Supabase RLS still controls which Clarity Circle rows are visible.</div>
    </main>
  </body>
</html>`;
}
...
export async function handleOAuth(req: IncomingMessage, res: ServerResponse) {
  ...
  if (requestUrl.pathname === "/oauth/authorize" && req.method === "POST") {
    ...
    try {
      ...
      const scope = params.get("scope") || "clarity_circle.read clarity_circle.write";
